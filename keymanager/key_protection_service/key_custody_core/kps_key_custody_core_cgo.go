...
import (
	...
	pb "github.com/google/go-tpm-tools/keymanager/workload_service/proto"
...
func GenerateKEMKeypair(bindingPubKey []byte, lifespanSecs uint64) (uuid.UUID, []byte, error) {
	...
	algo := &pb.HpkeAlgorithm{
		Kem:  pb.KemAlgorithm_KEM_ALGORITHM_DHKEM_X25519_HKDF_SHA256,
		Kdf:  pb.KdfAlgorithm_KDF_ALGORITHM_HKDF_SHA256,
		Aead: pb.AeadAlgorithm_AEAD_ALGORITHM_AES_256_GCM,
	}

	algoBytes, err := proto.Marshal(algo)
	if err != nil {
		return uuid.Nil, nil, fmt.Errorf("failed to marshal algo: %w", err)
	}
